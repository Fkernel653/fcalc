...
class Calc(customtkinter.CTk):
    def __init__(self):
        ...
        # Number 0 button
        self.button_0 = Button(self.button_frame, '0', lambda: self.input(0), style.selection)
        self.button_0.grid(row=4, column=0, columnspan=1, padx=5, pady=5, sticky='nsew')
        
        # Number 1 button
        self.button_1 = Button(self.button_frame, '1', lambda: self.input(1), style.selection)
        self.button_1.grid(row=3, column=0, columnspan=1, padx=5, pady=5, sticky='nsew')

        # Number 2 button
        self.button_2 = Button(self.button_frame, '2', lambda: self.input(2), style.selection)
        self.button_2.grid(row=3, column=1, columnspan=1, padx=5, pady=5, sticky='nsew')

        # Number 3 button
        self.button_3 = Button(self.button_frame, '3', lambda: self.input(3), style.selection)
        self.button_3.grid(row=3, column=2, columnspan=1, padx=5, pady=5, sticky='nsew')

        # Number 4 button
        self.button_4 = Button(self.button_frame, '4', lambda: self.input(4), style.selection)
        self.button_4.grid(row=2, column=0, padx=5, pady=5, sticky='nsew')
        
        # Number 5 button
        self.button_5 = Button(self.button_frame, '5', lambda: self.input(5), style.selection)
        self.button_5.grid(row=2, column=1, padx=5, pady=5, sticky='nsew')
        
        # Number 6 button
        self.button_6 = Button(self.button_frame, '6', lambda: self.input(6), style.selection)
        self.button_6.grid(row=2, column=2, padx=5, pady=5, sticky='nsew')

        # Number 7 button
        self.button_7 = Button(self.button_frame, '7', lambda: self.input(7), style.selection)
        self.button_7.grid(row=1, column=0, padx=5, pady=5, sticky='nsew')

        # Number 8 button
        self.button_8 = Button(self.button_frame, '8', lambda: self.input(8), style.selection)
        self.button_8.grid(row=1, column=1, padx=5, pady=5, sticky='nsew')

        # Number 9 button
        self.button_9 = Button(self.button_frame, '9', lambda: self.input(9), style.selection)
        self.button_9.grid(row=1, column=2, padx=5, pady=5, sticky='nsew')

        # Decimal point button
        self.button_dot = Button(self.button_frame, '.', lambda: self.input('.'), style.selection)
        self.button_dot.grid(row=4, column=1, padx=5, pady=5, sticky='nsew')


        # Addition button
        self.button_plus = Button(self.button_frame, '+', lambda: self.input('+'), style.pink)
        self.button_plus.grid(row=2, column=3, padx=5, pady=5, sticky='nsew')
 
        # Subtraction button
        self.button_minus = Button(self.button_frame, '-', lambda: self.input('-'), style.pink)
        self.button_minus.grid(row=1, column=3, padx=5, pady=5, sticky='nsew')

        # Multiplication button
        self.button_multiplication = Button(self.button_frame, '*', lambda: self.input('*'), style.pink)
        self.button_multiplication.grid(row=0, column=3, padx=5, pady=5, sticky='nsew')
        
        # Division button
        self.button_division = Button(self.button_frame, '/', lambda: self.input('/'), style.pink)
        self.button_division.grid(row=0, column=2, padx=5, pady=5, sticky='nsew')

        # Equals button (calculates result)
        self.button_equals = Button(self.button_frame, '=', lambda: self.answer(), style.purple)
        self.button_equals.grid(row=3, column=3, rowspan=2, padx=5, pady=5, sticky='nsew')

        # Backspace/Delete button
        self.button_backspace = Button(self.button_frame, 'DEL', lambda: self.backspace_call(), style.comment)
